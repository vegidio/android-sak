 ...
 *
 * Thread safety is provided by a [Mutex], mirroring Swift actor semantics from the iOS version.
 * Entries are evicted lazily on read (expired entry is removed and null is returned).
...
internal class ResponseCache(
    ...
    /**
     * Stores [body] under [key] with the configured TTL.
     * If [maxEntries] is reached, the oldest entry is evicted first.
     */
    suspend fun put(key: String, body: String): Unit = mutex.withLock {
    ...
    companion object {
        /**
         * Builds a cache key from a full URL string (including query parameters).
         * [okhttp3.HttpUrl.toString] already includes sorted, encoded query params.
         */
