...
internal object JwtUtility {
    ...
    fun expiryDate(token: String): Date? {
        return try {
            ...
            val exp = json.parseToJsonElement(payload).jsonObject["exp"]?.jsonPrimitive?.longOrNull ?: return null
    ...
    /**
     * Returns true if [token] expires within [threshold] from now, or if the expiry claim cannot be parsed (treated as
     * already expired).
     */
